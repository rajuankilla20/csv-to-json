...
public class CustomProductsUtil {
    ...
    public static void buildSecitonProducts(Map<SectionProductDto, ProductGpod> directProductsMap, Set<SectionProductDto> sectionProductNames,Set<ProductGpod> sectionProducts){

        sectionProductNames.forEach(productName-> {
            if(directProductsMap.containsKey(productName)){
                sectionProducts.add(directProductsMap.get(productName));
            }else{
    ...
    public static void buildSecitonProductSectionDto(String fileName, Set<SectionProductDto> sectionNames)  throws IOException {
        ...
        try (
            ...
            while ((row = csvReader.readNext()) != null) {
                ...
                SectionProductDto data = new SectionProductDto(row[4],row[10]);
                if(sectionNames.contains(data)){
                    System.out.println(" section dup names : " + data);
                    sectionNames.add(data);
                }else {
                    sectionNames.add(data);
                }
//                sectionNames.add(new SectionProductDto(row[4],row[10]));
...
//        System.out.println("--prodId "+ row[0]);
        ...
        if(categoriesMasterList.contains(row[1])){
            p.getCategories().add(new Category(row[1], row[1])); // 1- category
//            System.out.println("Found category : " + row[1]);
        }else{
            System.out.println("Not Found category : " + row[1] + " prod name " + row[5] );
        }

        if(subCategoriesMasterList.contains(row[2])){
            p.getSubCategories().add(new SubCategory(row[2], row[2])); // 2- sub-category
//            System.out.println("Found Subcategory : " + row[2]);
        }else{
            System.out.println("Not Found sub-category : " + row[2] + " prod name " + row[5] );
        }
...
//        if(brandsMasterList.contains(row[4])){
            p.getBrands().add(new Brand(row[4],row[4])); //4-brand
//            System.out.println("Found Brand : " + row[4]);
//        }else{
//            System.out.println("Not Found Brand : [ " + row[4]);
//        }
      ...
      if( (row[13].charAt(0) == 'X' || row[13].charAt(0) == 'x') || (row[13].equalsIgnoreCase("RichardsonsFarm.jpg")) || (existingImages.contains(row[13])) ){
          ...
          System.out.println("--No image set for "+ row[5]);
        }
